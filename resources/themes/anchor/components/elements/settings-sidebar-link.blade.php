<a href="{{ $href }}"
    ...
    <span
        class="@if ($href == RalphJSmit\Livewire\Urls\Facades\Url::current()) {{ 'w-full h-1 lg:h-5/6' }}@else{{ 'h-0' }} @endif absolute bottom-0 left-0 block w-full translate-y-2 rounded-full transition-all duration-300 ease-out lg:top-1/2 lg:w-1 lg:-translate-x-1.5 lg:-translate-y-1/2"
