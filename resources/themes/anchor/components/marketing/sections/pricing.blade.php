<section>
    <x-marketing.elements.heading level="h2" title="Chart Your Course"
    ...
    <div x-data="{
        ...
        @if (has_monthly_yearly_toggle())
            <div class="relative flex -translate-y-2 items-center justify-start pb-5 md:justify-center">
                <div
                    class="relative inline-flex w-auto -translate-y-3 items-center justify-center rounded-full border-2 border-zinc-900 p-1 text-center md:mx-auto">
                    <div x-ref="monthly" x-on:click="billing='Monthly'; toggleRepositionMarker($el)"
                        :class="{ 'text-white': billing == 'Monthly', 'text-zinc-900': billing != 'Monthly' }"
                        class="relative z-20 cursor-pointer rounded-full px-3.5 py-1 text-sm font-medium leading-6 duration-300 ease-out">
                        Monthly
                    </div>
                    <div x-ref="yearly" x-on:click="billing='Yearly'; toggleRepositionMarker($el)"
                        :class="{ 'text-white': billing == 'Yearly', 'text-zinc-900': billing != 'Yearly' }"
                        class="relative z-20 cursor-pointer rounded-full px-3.5 py-1 text-sm font-medium leading-6 duration-300 ease-out">
                        Yearly
                    </div>
        ...
        <div class="flex flex-col flex-wrap lg:flex-row lg:space-x-5">
            ...
            @foreach (Wave\Plan::where('active', 1)->get() as $plan)
                ...
                <div {{--  Say that you have a monthly plan that doesn't have a yearly plan, in that case we will hide the place that doesn't have a price_id --}}
                    x-show="(billing == 'Monthly' && '{{ $plan->monthly_price_id }}' != '') || (billing == 'Yearly' && '{{ $plan->yearly_price_id }}' != '')"
                    class="mx-auto mb-6 w-full flex-1 px-0 md:max-w-lg lg:mb-0" x-cloak>
                    <div
                        class="@if ($plan->default) {{ 'border-zinc-900 lg:scale-105' }}@else{{ 'border-zinc-200' }} @endif flex h-full flex-col rounded-xl border-2 bg-white shadow-sm sm:mb-0 lg:mb-10">
                        <div class="px-8 pt-8">
                            <span
                                class="text-uppercase rounded-full bg-zinc-900 px-4 py-1 text-base font-medium text-white"
                        ...
                        <div class="mt-5 px-8">
                            <span class="text-5xl font-bold">$<span
                                    x-text="billing == 'Monthly' ? '{{ $plan->monthly_price }}' : '{{ $plan->yearly_price }}'"></span></span>
                            <span class="text-xl font-bold text-zinc-500"><span
                                    x-text="billing == 'Monthly' ? '/mo' : '/yr'"></span></span>
                        </div>
                                ...
                                @foreach ($features as $feature)
                                    <li class="mt-1">
                                        <span class="flex items-center text-green-500">
                                            <svg class="mr-3 h-4 w-4 fill-current" xmlns="http://www.w3.org/2000/svg"
                                                viewBox="0 0 20 20">
