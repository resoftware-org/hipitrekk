...
<div x-data="{ dropdownOpen: false }" :class="{ 'block z-50 w-auto lg:w-full dark:bg-zinc-900 dark:border-zinc-800': open, 'hidden': !open }" class="relative flex-shrink-0 dark:text-zinc-200 sm:flex sm:w-auto sm:items-center sm:bg-transparent sm:p-0" x-cloak>
    <button @click="dropdownOpen=!dropdownOpen" class="group-hover:autoflow-auto items flex w-full w-full items-center justify-between space-x-1 space-x-1.5 overflow-hidden rounded-lg p-2.5 text-[13px] hover:bg-zinc-200/70 hover:text-black dark:hover:bg-zinc-700/60 dark:hover:text-zinc-100 lg:p-2">
        ...
        <svg :class="{ 'rotate-180': '{{ $position }}' == 'bottom' }" class="relative right-0 mr-4 h-4 w-4 -translate-x-0.5 fill-current duration-0 ease-out group-hover:delay-150 group-hover:duration-300" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" />
        </svg>
    </button>
    <div wire:ignore x-show="dropdownOpen" @mouse.leave="dropdownOpen=false" @click.away="dropdownOpen=false" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 sm:scale-95" x-transition:enter-end="transform opacity-100 sm:scale-100"
        x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 sm:scale-100" x-transition:leave-end="transform opacity-0 sm:scale-95" @class([
            'z-50',
        ...
        <div class="mt-1 rounded-xl border border-zinc-200/70 bg-white pt-0 text-zinc-600 shadow-md dark:border-white/10 dark:border-zinc-700 dark:bg-zinc-900 dark:text-white/70 dark:shadow-xl sm:space-y-0.5 sm:border">
