...
});" id="confirmCancel" x-show="$store.confirmCancel.open"
    class="fixed inset-0 z-50 overflow-y-auto" x-cloak>
    <div class="flex min-h-screen items-end justify-center px-4 pb-20 pt-4 text-center sm:block sm:p-0">
        <div x-show="$store.confirmCancel.open" x-on:click="$store.confirmCancel.open = false"
        ...
        <div x-show="$store.confirmCancel.open" x-transition:enter="ease-out duration-300"
            ...
            <div class="mt-2 flex w-full flex-col justify-between">
                ...
                <div class="mt-5 sm:mt-6 sm:flex sm:flex-row-reverse">
                    <span class="flex w-full flex-1 rounded-md shadow-sm sm:ml-3 sm:w-full">
                        <div data-url="{{ auth()->user()->subscription->cancel_url }}"
                            @click="$store.confirmCancel.open=false"
