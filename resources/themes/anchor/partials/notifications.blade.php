...
<div class="relative w-full">
    @forelse ($unreadNotifications as $index => $notification)
        ...
        <div id="notification-li-{{ $index + 1 }}" class="@if (!$loop->last) {{ 'border-b' }} @endif flex flex-col border-zinc-200 pb-5">
            ...
            <span wire:click="delete('{{ $notification->id }}')" class="k mark-as-read ml-1 flex w-full cursor-pointer justify-start py-1 pl-16 text-xs text-zinc-500 hover:text-zinc-700 hover:underline">
                <svg class="absolute mr-1 mt-1 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    @empty
        <div id="notifications-none" class="@if ($notifications_count > 0) {{ 'hidden' }} @endif flex h-24 w-full items-center justify-center font-medium text-zinc-600">
