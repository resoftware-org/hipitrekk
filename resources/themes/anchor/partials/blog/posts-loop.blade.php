...
@foreach ($posts as $post)
    <article id="post-{{ $post->id }}" class="relative col-span-2 cursor-pointer overflow-hidden rounded-2xl border border-zinc-100 border-zinc-200 bg-white p-4 dark:bg-black sm:col-span-1" onClick="window.location='{{ $post->link() }}'">
        ...
        <div class="px-1 py-1">
            <div class="my-3 flex items-center gap-x-4 text-xs">
                ...
                <a href="/blog/{{ $post->category->slug }}" class="relative z-10 rounded-full bg-zinc-50 px-3 py-1.5 font-medium text-zinc-600 hover:bg-zinc-100">{{ $post->category->name }}</a>
            </div>
